...
function DashboardCard17({ selectedDate }) {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      ...
      axios.get(`http://localhost:8080/api/v1/user/facebook/pages/page-views_total?since=${formattedSinceDate}&until=${formattedUntilDate}`, {
        ...
        const pageView = result.data[0].pageView;
        setTotal(pageView.totalView);
        const labels = pageView.viewByDayList.map((entry) => entry.date);
        const data = pageView.viewByDayList.map((entry) => entry.value);
        setChartData({
          labels: labels,
          datasets: [
            {
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      <header className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center">
        <h2 className="font-semibold text-slate-800 dark:text-slate-100">Page Views</h2>
      </header>
